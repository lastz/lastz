...
//#define debugPosA1 15384592	// if defined (and snoopAlgorithm is also),
//#define debugPosA2 11569819	// .. only information about extension of this
//#define debugPosB1 130		// .. particular seed is output;  note that
//#define debugPosB2 136		// .. this position is origin-zero
...
typedef struct galign
	...
	u64			hspId;			// (for debugging) uniquely identifies the
								// .. hsp that led to this alignment attempt
...
// flag related to reprting of "truncated alignments"

static int haveReportedTruncation = false;
   ...
   (seq*		seq1,
	...
	if (gapped_extend_dbgShowAnchors)
		...
		if (anchors->len == 0)
			...
			fprintf (stderr, "reduce_to_points: %s anchors\n",
	...
	for (ix=0,seg=anchors->seg ; ix<anchors->len ; ix++,seg++)
		...
		if (gapped_extend_dbgShowAnchors)
			{
			reportAnchor = ((gapped_extend_dbgShowAnchorsHowOften == 0)
		                 || (ix % gapped_extend_dbgShowAnchorsHowOften == 0));
			if (reportAnchor)
				fprintf (stderr, "reduce_to_points: reducing [%s] ("
...
#define debugSnoopAnchors_1                                                  \
	...
	for (ix=0 ; ix<anchors->len ; ix++)                                      \
		...
		fprintf (stderr,"anchors[%3d,hspid=" u64Fmt "] " unsposSlashFmt " " unsposFmt " " scoreFmt, \
		         ix, anchors->seg[ix].hspId,                                 \
		             anchors->seg[ix].pos1+1,                                \
...
static sbtable* batched_segments
   (segtable*		anchors,
	...
	//            one through the entire program;  that may change in the future
...
static galign** init_from_anchors
   (segtable*	anchors,
	...
	if ((u32max-16) / sizeof(galign) < (u32) numSlots) goto overflow1;
	bytesBlocks   =  round_up_16 (numSlots * sizeof(galign));
	bytesNeeded   =  bytesPointers;
	bytesNeeded   += bytesBlocks;  if (bytesNeeded < bytesBlocks) goto overflow2;
...
#define suggestions " consider raising scoring threshold (--hspthresh or --exact)" \
                    " or breaking your target sequence into smaller pieces"

overflow1:
	suicidef ("in init_from_anchors(), structure size would exceed 2^32"
	          " (%u + %u*%u)\n"
	          suggestions,
	          bytesPointers, sizeof(galign), numSlots);
	return NULL; // (doesn't get here)

overflow2:
	suicidef ("in init_from_anchors(), structure size exceeds 2^32"
	          " (%u + %u)\n"
	          suggestions,
...
static void lop_initial_indels (alignio* io);
static void lop_final_indels   (alignio* io);
...
static void ydrop_align
   (alignio*	io)
	...
	// in rare cases, the DP algorithm can produce an alignment that starts or
	// ends with a gap;  remove those here
	...
	if (io->script->len != 0)
		...
		if (op != editopSub)
			...
			// nota bene: lop_initial_indels will not set len to zero
...
static void lop_initial_indels
   (alignio*	io)
	{
	unspos		pos1, pos2;
	u32      	opIx, op, rpt, numIndelOps;

	// scan past all indel ops at the start

	pos1 = io->start1;
	pos2 = io->start2;

	for (opIx=0 ; opIx<io->script->len ; opIx++)
		{
		op  = edit_op_operation (io->script->op[opIx]);
		rpt = edit_op_repeat    (io->script->op[opIx]);
		if      (op == editopSub) break;
		else if (op == editopIns) pos2 += rpt;
		else if (op == editopDel) pos1 += rpt;
		}

	numIndelOps = opIx;

	// special case if the alignment was nothing but indels;  assign it a
	// phenominally bad score so that something downstream from us will discard
	// it

	if (numIndelOps == io->script->len)
		{
		io->s = worstPossibleScore;
		return;
		}

	// modify the starting position, copy the remainder of the edit script
	// forward, and rescore the alignment

	io->start1 = pos1;
	io->start2 = pos2;

	io->script->len -= numIndelOps;
	for (opIx=0 ; opIx<io->script->len ; opIx++)
		io->script->op[opIx] = io->script->op[opIx+numIndelOps];

	io->s = score_alignment (io->scoring,
	                         io->seq1, io->start1,
	                         io->seq2, io->start2,
	                         io->script);
	}
...
static void lop_final_indels
   (alignio*	io)
	{
	unspos		pos1, pos2;
	u32      	opIx, op, rpt;

	// scan past all indel ops at the end

	pos1 = io->stop1;
	pos2 = io->stop2;

	for (opIx=io->script->len ; opIx>0 ; )
		{
		opIx--;
		op  = edit_op_operation (io->script->op[opIx]);
		rpt = edit_op_repeat    (io->script->op[opIx]);
		if      (op == editopSub) { opIx++;  break; }
		else if (op == editopIns) pos2 -= rpt;
		else if (op == editopDel) pos1 -= rpt;
		}
	...
	// modify the ending position, truncate the edit script, and rescore the
	// alignment
...
//	editscript** script:	Place to return the resulting edit script.  Note
//							.. that this is in traceback order, with the
//							.. alignment's starting point corresponding to the
//							.. final operation in the edt script.
...
#define snoopAlgorithm_5A                                                    \
	if (snoop)                                                               \
		...
		fprintf (stderr, " **new best score %s**", ccSnoop);                 \
		}
...
static unspos special_min(unspos RY, sgnpos R);
static unspos special_min(unspos RY, sgnpos R)
	{
	// ideally we would return min(RY,R);  but R is signed, so if R is negative
	// we need to return zero (since we are to return an unsigned);  if R is
	// positive then we can return min(RY,R), but we may as well compute it
	// ourselves (instead of calling min);  note that the cast of R to unsigned
	// only occurs when R is positive, so we needn't worry about it causing
	// overflaw (and yes, I mean overflaw not overflow)

	if      (R <= 0)          return 0;
	else if ((unspos) R < RY) return R;
	else                      return RY;
	}
...
u32 gappily_extend_hsps
   (void*	_info,
	...
	if (gapped_extend_dbgShowAnchors)
		...
		write_segments (stderr, &st, seq1, seq2, false, gapped_extend_dbgShowAnchorsHowOften);
		}
	...
	// subject the alignment to the user's guantlet of filtering settings
	...
	// if we're to prevent duplicates, compute the alignment's hash and make
	// sure we haven't already seen it

	if (info->alignmentHashes != NULL)
		{
		// if the list is already *past* full, we don't need to check whether
		// this alignment is a duplicate, we can just reject it

		if (info->alignmentHashesSeen > info->alignmentHashesSize)
			goto return_zero;

		// compute the hash value and check if we've seen it before;  note that
		// if the list is *just* full, we still need to check, and only reject
		// if it is truly a duplicate;  this allows the calling routine to
		// properly detect when the number of alignments exceeds the limit

		h = alignment_hash (mp.align->beg1, mp.align->end1, seq1->revCompFlags,
		                    mp.align->beg2, mp.align->end2, seq2->revCompFlags,
		                    /*script*/ NULL);

		for (hIx=0 ; hIx<info->alignmentHashesSeen ; hIx++)
			{
			if (hIx >= info->alignmentHashesSize) break;
			if (info->alignmentHashes[hIx] == h)
				goto return_zero;
			}

		hIx = info->alignmentHashesSeen++;
		if (hIx >= info->alignmentHashesSize)
			goto return_one;

		info->alignmentHashes[hIx] = h;
		}

	// the alignment has satisfied all the user's criteria-- print it
